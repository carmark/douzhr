...
def tip_pre_save(signal, instance, sender, **kwargs):
    ...
    while Zhishi.objects.filter(slug_title=new_slug).exclude(id=instance.id).count() > 0:
        counter += 1
        new_slug = '%s-%d'%(slug, counter)

    instance.slug_title = new_slug
signals.pre_save.connect(tip_pre_save, sender=Zhishi)
    

class Bookmark(models.Model):
    user = models.ForeignKey(User)
    zhishi = models.ForeignKey(Zhishi)
